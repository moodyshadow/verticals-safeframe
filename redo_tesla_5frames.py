...
from verticals.broll import (
    _generate_image_img2img_local_sd, _generate_video_local_animatediff,
    _generate_image_local_sd, _vision_qa_frame,
)
...
def img2img_frame(ref_path, prompt, out_path):
    ...
    for attempt in range(3):
        _generate_image_img2img_local_sd(prompt, ref_bytes, out_path, denoising_strength=0.6, seed=random.randint(0, 2**31 - 1))
...
def fresh_video_frame(prompt, out_path):
    ok = _generate_video_local_animatediff(prompt, out_path)
    print(f"  AnimateDiff success: {ok}")
    if ok:
        return out_path
    # fall back to still
    still_path = out_path.with_suffix(".png")
    qa_passed = False
    for attempt in range(3):
        _generate_image_local_sd(prompt, still_path, seed=random.randint(0, 2**31 - 1))
        qa_passed, reason = _vision_qa_frame(still_path)
        print(f"  still attempt {attempt+1}: passed={qa_passed} reason={reason}")
        if qa_passed:
            break
    resize_crop(still_path, still_path)
    return still_path

frames = draft["_pipeline_state"]["broll"]["artifacts"]["frames"]

print("Frame 1 (camera system)...")
p = img2img_frame(
    ...
    WORK_DIR / "broll_0_v2.png",
)
frames[0] = str(p)

print("Frame 4 (minimalist product shot)...")
p = img2img_frame(
    ...
    WORK_DIR / "broll_3_v2.png",
)
frames[3] = str(p)

print("Frame 8 (wide city street)...")
p = img2img_frame(
    ...
    WORK_DIR / "broll_7_v2.png",
)
frames[7] = str(p)

print("Frame 3 (server room, fresh, no reuse)...")
p = fresh_video_frame(
    "A server room with blue LED lighting, with rows of computer servers humming in the background, one continuous scene, no collage, no split screen, gaming aesthetic, dramatic lighting, high contrast, cinematic, 4K quality",
    WORK_DIR / "broll_2_v2.mp4",
)
frames[2] = str(p)

print("Frame 5 (circuit board, fresh, no reuse)...")
p = fresh_video_frame(
    "A close-up of a circuit board with glowing nodes, representing complex AI systems, one continuous scene, no collage, no split screen, dramatic lighting, high contrast, cinematic, 4K quality",
    WORK_DIR / "broll_4_v2.mp4",
)
frames[4] = str(p)

# Clean up old files
for old_name in ["broll_0.mp4", "broll_0.png", "broll_2.mp4", "broll_3.mp4", "broll_3.png", "broll_4.mp4", "broll_7.mp4", "broll_7.png"]:
